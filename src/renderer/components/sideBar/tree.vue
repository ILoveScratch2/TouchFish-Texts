  ...
  <div class="tree-view">
    ...
    <div class="opened-files">
      <div class="title">
        ...
        <span class="default-cursor text-overflow" @click.stop="toggleOpenedFiles()">{{ $t('sideBar.openedFiles') }}</span>
        <a href="javascript:;" @click.stop="saveAll(false)" :title="$t('sideBar.saveAll')">
          <svg class="icon" aria-hidden="true">
            <use xlink:href="#icon-save-all"></use>
          </svg>
        </a>
        <a href="javascript:;" @click.stop="saveAll(true)" :title="$t('sideBar.closeAll')">
          <svg class="icon" aria-hidden="true">
    ...
    >
      ...
      <div class="tree-wrapper" v-show="showDirectories">
        ...
        <div class="empty-project" v-if="projectTree.files.length === 0 && projectTree.folders.length === 0">
          <span>{{ $t('sideBar.emptyProject') }}</span>
          <a href="javascript:;" @click.stop="createFile">{{ $t('sideBar.createFile') }}</a>
        </div>
      </div>
    </div>
