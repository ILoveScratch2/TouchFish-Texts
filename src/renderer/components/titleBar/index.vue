  ...
  <div>
    ...
    >
      <div class="title" @dblclick.stop="toggleMaxmizeOnMacOS">
        <span v-if="!filename">TouchFish Texts</span>
        <span v-else>
          <span
...
export default {
  ...
  watch: {
    filename: function (value) {
      ...
      let title = ''
      if (value) {
        title = hasOpenFolder ? `${value} - ${this.project.name}` : `${value} - TouchFish Texts`
      } else {
        title = hasOpenFolder ? this.project.name : 'TouchFish Texts'
      }
