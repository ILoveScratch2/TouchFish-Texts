<template>
  <div class="pref-spellchecker">
    <h4>{{ $t('prefSpelling.title') }}</h4>
    <compound>
      <template #head>
        <bool
          :description="$t('prefSpelling.enabled')"
      ...
      <template #children>
        <bool
          :description="$t('prefSpelling.noUnderline')"
        ...
        <bool
          v-show="isOsx"
          :description="$t('prefSpelling.autoDetect')"
          :bool="true"
          :disable="true"
        ></bool>
        <cur-select
          v-show="!isOsx"
          :description="$t('prefSpelling.language')"
    ...
    <div v-if="isOsx && spellcheckerEnabled" class="description">
      {{ $t('prefSpelling.macOsNote') }}
    </div>

    <div v-if="!isOsx && spellcheckerEnabled">
      <h6 class="title">{{ $t('prefSpelling.customDictTitle') }}</h6>
      <div class="description">{{ $t('prefSpelling.editCustomDict') }}</div>
      <el-table
        :data="wordsInCustomDictionary"
        :empty-text="$t('prefSpelling.noWords')"
        style="width: 100%"
      >
        <el-table-column prop="word" :label="$t('prefSpelling.wordColumn')">
        </el-table-column>

        <el-table-column fixed="right" :label="$t('prefSpelling.optionsColumn')" width="90">
          <template slot-scope="scope">
            <el-button @click="handleDeleteClick(scope.row)" type="text" size="small" :title="$t('prefSpelling.deleteWord')">
    ...
    handleSpellcheckerLanguage (languageCode) {
      this.ensureDictLanguage(languageCode)
        ...
        .catch(error => {
          log.error(error)
          notice.notify({
            title: this.$t('prefSpelling.errorSwitchLanguage'),
            type: 'error',
        ...
        ipcRenderer.invoke('mt::spellchecker-remove-word', selectedItem.word)
          .then(success => {
            if (success) {
              ...
              notice.notify({
                title: this.$t('prefSpelling.errorRemoveWord'),
                type: 'error',
                message: this.$t('prefSpelling.errorSave')
              })
            }
          })
