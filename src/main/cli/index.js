...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to TouchFish Texts and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'touchfish-texts-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: touchfish-texts [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`TouchFish Texts: ${global.TOUCHFISH_TEXTS_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'touchfish-texts-user-data')
