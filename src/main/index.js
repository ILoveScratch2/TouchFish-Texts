...
// Make TouchFish Texts a single instance application.
if (!process.mas && process.env.NODE_ENV !== 'development') {
  const gotSingleInstanceLock = app.requestSingleInstanceLock()
  if (!gotSingleInstanceLock) {
    process.stdout.write('Other TouchFish Texts instance detected: exiting...\n')
    app.exit()
  }
}

// TouchFish Texts environment is configured successfully. You can now access paths, use the logger etc.
...
try {
  ...
  const msgHint = err.message.includes('Config schema violation')
    ...
    : ''
  log.error(`Loading TouchFish Texts failed during initialization! ${msgHint}`, err)

  const EXIT_ON_ERROR = !!process.env.TOUCHFISH_TEXTS_EXIT_ON_ERROR
  const SHOW_ERROR_DIALOG = !process.env.TOUCHFISH_TEXTS_ERROR_INTERACTION
...
const touchFishTexts = new App(accessor, args)
touchFishTexts.init()
